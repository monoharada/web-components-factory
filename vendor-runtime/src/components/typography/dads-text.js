/**
 * dads-text コンポーネント
 * デジタル庁タイポグラフィシステムの基本テキストコンポーネント
 * Shape Up Week 1: 最小限の実装
 */
import { html, css, PropertyAttr } from '../../core/web-components.js';
import { TypographyWebComponent } from '../../core/typography/typography-web-component.js';
import { typographyTokens } from '../../styles/design-tokens/typography-tokens.js';
import { setDefaultAttributes } from '../../utils/form-component-helpers.js';
/**
 * 基本テキストコンポーネント
 *
 * @customElement dads-text
 * @tagname dads-text
 *
 * @slot default - テキストコンテンツ
 *
 * @csspart text - テキストラッパー
 *
 * @attr {string} variant - バリアント（standard | display | dense）
 * @attr {string} size - サイズ（16 | 20 | 32）
 * @attr {string} weight - 太さ（normal | bold）
 * @attr {string} display - 表示（inline | block）
 */
export class DadsText extends TypographyWebComponent {
    connectedCallback() {
        super.connectedCallback();
        setDefaultAttributes(this, { variant: 'standard', display: 'inline' });
    }
    variantChanged(_oldValue, newValue) {
        const validVariants = ['standard', 'display', 'dense'];
        if (newValue === null || !validVariants.includes(newValue)) {
            if (this.getAttribute('variant') !== 'standard') {
                this.setAttribute('variant', 'standard');
            }
        }
    }
    sizeChanged(_oldValue, newValue) {
        const validSizes = ['16', '20', '32'];
        if (newValue === null)
            return;
        if (!validSizes.includes(newValue)) {
            this.removeAttribute('size');
        }
    }
    weightChanged(_oldValue, newValue) {
        const validWeights = ['normal', 'bold'];
        if (newValue === null)
            return;
        if (!validWeights.includes(newValue)) {
            this.removeAttribute('weight');
        }
    }
    displayChanged(_oldValue, newValue) {
        if (newValue === null || (newValue !== 'inline' && newValue !== 'block')) {
            if (this.getAttribute('display') !== 'inline') {
                this.setAttribute('display', 'inline');
            }
        }
    }
}
DadsText.definition = {
    name: 'dads-text',
    template: html `
      <span part="text">
        <slot></slot>
      </span>
    `,
    styles: [
        typographyTokens,
        css `
        :host {
          display: inline;
          font-family: var(--dads-text-font-family);
          font-size: var(--dads-text-font-size);
          font-weight: var(--dads-text-font-weight);
          line-height: var(--dads-text-line-height);
          color: var(--dads-text-color);
          letter-spacing: var(--dads-text-letter-spacing);
          text-align: var(--dads-text-text-align);
        }
        
        :host([display="block"]) {
          display: block;
        }
        
        [part="text"] {
          display: contents;
        }
      `
    ],
    attributes: [
        PropertyAttr('variant'),
        PropertyAttr('size'),
        PropertyAttr('weight'),
        PropertyAttr('display')
    ]
};
DadsText.define();
