 ...
 */
export function setDefaultAttributes(element, defaults) {
 ...
 */
export function findParentForm(element, internals) {
    // 優先: _internals.form
    let form = internals.form;
    if (!form) {
        // フォールバック: DOM走査
 ...
 */
export function setupFormValidation(element, internals, autoValidateAttr, onSubmit) {
    let boundForm = null;
 ...
 */
export function updateLabelFallback(labelSlot, labelFallback, labelAttr) {
    if (!labelFallback)
        return;
 ...
 */
export function updateSupportFallback(supportSlot, supportText, supportFallback, supportAttr) {
    if (!supportText || !supportFallback)
        return;
 ...
 */
export function updateErrorFallback(errorSlot, errorText, errorFallback, errorAttr, hasError) {
    if (!errorText || !errorFallback)
        return;
 ...
 */
export function updateRequirement(requirement, isRequired, isReadonly) {
    if (!requirement)
        return;
 ...
 */
export function updateValidationUI(control, hasError, updateErrorFn, updateAriaFn) {
    updateErrorFn();
    updateAriaFn();
    if (control) {
 ...
 */
export function showValidationError(options) {
 ...
 */
export function clearValidationError(element, internals, updateUI) {
 ...
 * @param config - ID設定
 */
export function updateAriaDescribedBy(control, supportVisible, errorVisible, counterVisible = false, config = {}) {
    if (!control)
        return;
    const { supportTextId = 'support-text', errorTextId = 'error-text', counterId = 'counter', } = config;
    const ids = [];
    if (supportVisible) {
 ...
 */
export function setupSlotChangeListeners(slots, handlers) {
