...
_ViewTemplate_html = new WeakMap();
...
export class WebComponent extends HTMLElement {
    static get observedAttributes() {
        const def = WebComponentDefinition.forType(this) ??
            WebComponentDefinition.compose(this, {
                name: 'dummy',
            });
...
_WebComponent_view = new WeakMap(), _WebComponent_def = new WeakMap(), _WebComponent_init = new WeakMap(), _WebComponent_sr = new WeakMap(), _WebComponent_dsd = new WeakMap(), _WebComponent_instances = new WeakSet(), _WebComponent_setup = function _WebComponent_setup() {
    ...
    if (!__classPrivateFieldGet(this, _WebComponent_dsd, "f") && styles.length > 0) {
        const mergeSheets = (current, next) => {
            const base = (() => {
        ...
        if (__classPrivateFieldGet(this, _WebComponent_sr, "f")) {
            ...
            __classPrivateFieldGet(this, _WebComponent_sr, "f").adoptedStyleSheets = mergeSheets(__classPrivateFieldGet(this, _WebComponent_sr, "f").adoptedStyleSheets, styles);
        }
        else {
            // Light DOMの場合
            const root = this.getRootNode();
            // happy-dom等の環境では adoptedStyleSheets が配列ではない場合がある
            root.adoptedStyleSheets = mergeSheets(root.adoptedStyleSheets, styles);
        }
    }
...
export class ElementSelection {
    ...
    forEach(cb) {
        let idx = 0;
        for (const el of __classPrivateFieldGet(this, _ElementSelection_elements, "f")) {
            cb(el, el === __classPrivateFieldGet(this, _ElementSelection_current, "f"), idx);
            idx += 1;
        }
    }
    processKey(event, cb, orientationOrOptions = Orientation.vertical) {
        const total = __classPrivateFieldGet(this, _ElementSelection_elements, "f").length;
        if (total === 0)
            return;
        // Usage: new ElementSelection(items, current).processKey(event, (next) => next.focus(), { wrap: true });
        const options = typeof orientationOrOptions === 'string'
            ? { orientation: orientationOrOptions }
            : orientationOrOptions;
        const { orientation = Orientation.vertical, wrap = false, allowAlternateAxis = false, preventDefaultHomeEnd = false, } = options;
        const dir = getComputedStyle(__classPrivateFieldGet(this, _ElementSelection_current, "f")).direction;
        const isVertical = orientation === Orientation.vertical;
        const isRtl = dir === 'rtl';
        const keySet = isVertical
        ...
        const isPrevKey = event.key === primaryPrev || (allowAlternateAxis && event.key === altPrev);
        const isNextKey = event.key === primaryNext || (allowAlternateAxis && event.key === altNext);
        const currentIndex = Math.max(0, this.currentIndex);
        const resolveIndex = wrap
            ? (index) => (index + total) % total
            : (index) => __classPrivateFieldGet(this, _ElementSelection_instances, "m", _ElementSelection_clamp).call(this, index);
        let target;
        if (isPrevKey) {
            event.preventDefault();
            target = __classPrivateFieldGet(this, _ElementSelection_elements, "f")[resolveIndex(currentIndex - 1)];
        }
        else if (isNextKey) {
            event.preventDefault();
            target = __classPrivateFieldGet(this, _ElementSelection_elements, "f")[resolveIndex(currentIndex + 1)];
...
_ElementSelection_elements = new WeakMap(), _ElementSelection_current = new WeakMap(), _ElementSelection_instances = new WeakSet(), _ElementSelection_clamp = function _ElementSelection_clamp(i) {
    if (i < 0)
        return 0;
    if (i >= __classPrivateFieldGet(this, _ElementSelection_elements, "f").length)
        return __classPrivateFieldGet(this, _ElementSelection_elements, "f").length - 1;
    return i;
};
