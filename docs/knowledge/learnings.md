...
## [2026-02-10] モバイルメニューの右/左タイプでトグルボタンが重なる問題と安定化パターン
**タグ**: #webcomponents #mobile-menu #drawer #a11y #debug

### 概要
右/左タイプのモバイルメニューで、単一トリガーを `menu/close` へ動的切替する構成は、ドロワー内部コンテンツと視覚的に重なりやすかった。`開くボタン（モック側）` と `閉じるボタン（ドロワー内）` を分離することで、左右配置とフォーカス視認性を安定化できた。

### 詳細
#### 症状
- 左タイプで `閉じる` ボタンが先頭メニュー項目に重なる
- 画面状態によって `閉じる` が見えない、または押しにくい
- フォーカスリングがコンテナ端で欠ける

#### 原因
- 1つのトリガーを常時オーバーレイ表示し、開閉状態だけを切り替えると、配置・重なり・余白が状態依存になりやすい
- ドロワー側とモック側で余白責務が分散し、クリック領域と視覚位置がずれる

#### 修正
- C/D 作例を `開くボタン（外側）+ 閉じるボタン（ドロワー内）` の2点構成へ変更
- 右タイプは右寄せ、左タイプは左寄せで固定
- ドロワー内に `close-row` を明示し、`dads-mobile-menu` との間に常に余白を確保
- `dads-mobile-menu` の区切り線は `dads-divider` に統一し、`hr` 前提スタイルを削除

### 再発防止
- モック重ね表示で「同一ボタンを開閉両方に使う」場合は、先に重なり検証を行う
- コンテナ端でフォーカスリングが見切れる UI は、内側余白で受ける設計に統一する
- 区切り線は `dads-divider` を使い、余白は `--dads-divider-margin-inline` 系で制御する

### 注意点
- `dads-drawer` のヘッダーを非表示にする場合、代替の閉じる操作を必ず同階層に配置する
- 左右タイプの差分は「配置（left/right）」と「余白バリエーション」に限定し、情報設計は共通化する

## [2026-02-10] single-toggle 構成で外側トリガーを明示的に隠す理由
**タグ**: #webcomponents #mobile-menu #drawer #debug

### 概要
`data-mobile-menu-single-toggle-button` 構成では、ドロワー開時に外側トリガーを `hidden` 切替しないと、環境によってはメニュー先頭項目と重なって見える。

### 詳細
#### 症状
- 右/左タイプで `閉じる` と先頭メニュー項目が視覚的に重なる
- 外側トグルが残って「閉じるボタンが2つあるように見える」状態になる

#### 原因
- 開閉同期を `aria-expanded` 更新だけにしていたため、外側トリガーの表示状態が開時にも残るケースがあった
- `:has(.drawer[open])` だけに依存すると、実行順序やレンダリングタイミングで取りこぼしが発生しうる

#### 修正
- single-toggle 分岐で `trigger.toggleAttribute('hidden', isOpen)` を実行し、JS側でも表示状態を確定
- ドロワー内 `close` トリガーは常設し、`aria-expanded` の同期のみを担当させる

### 再発防止
- オーバーレイUIで重なりが発生しうる要素は、CSS依存に加えてJSでも最終表示状態を確定させる
- `open/close` の視覚トリガーを別要素で運用する場合は、片方を `hidden` で排他にする
