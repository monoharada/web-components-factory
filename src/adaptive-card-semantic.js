...
class AdaptiveCard extends HTMLElement {
  ...
  static get observedAttributes() {
    return ['href', 'link-text', 'link-target'];
  }
  ...
  setupTemplate() {
    this.shadowRoot.innerHTML = `
      <style>
        ...
        /* Removed basic card focus - only interactive cards should be focusable */
  ...
  setupSlotObservers() {
    ...
    slots.forEach(slot => {
      slot.addEventListener('slotchange', () => {
        ...
        if (slot.name === 'actions' && this.hasAttribute('href')) {
          this.setupStretchedLink();
        }
      });
  ...
  setupKeyboardNavigation() {
    ...
    this.addEventListener('keydown', (event) => {
      if (event.key === 'Enter' || event.key === ' ') {
        ...
        if (activeElement === this && this.hasAttribute('href')) {
  ...
  setupStretchedLink() {
    ...
    if (href && this.validateURL(href)) {
      ...
      if (hasInteractiveButtons) {
        ...
        console.log('🔘 Card has interactive buttons - skipping stretched link creation');
      ...
      // Add to shadow DOM
      const article = this.shadowRoot.querySelector('.card-article');
      if (article) {
      ...
      this.dispatchEvent(new CustomEvent('card-link-setup', {
        detail: { href, linkText, linkTarget, hasButtons: false },
        bubbles: true
      }));
    }
  }
