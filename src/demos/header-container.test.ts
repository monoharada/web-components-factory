...
describe('header-container demo', () => {
  ...
  it('デバイスモック再利用（desktop/tablet/mobile + dads-drawer + command-store bind）を含む', () => {
    ...
    expect(html).toContain('class="header-container-demo__layout-device" device="tablet" visible-height="170px"');
    ...
    expect(html).toContain('var bindDrawerPair = function(drawerId, triggerId, triggerLayerId) {');
