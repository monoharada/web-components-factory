...
const SORT_ICON_ASC = svgUrl(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
    <path d="M8 2l3 3H9v9H7V5H5l3-3z"/>
  </svg>`
);

const SORT_ICON_DESC = svgUrl(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
    <path d="M8 14l-3-3h2V2h2v9h2l-3 3z"/>
  </svg>`
);

const MENU_ICON_ELLIPSIS = svgUrl(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
...
const SELECTION_CHECKBOX_SELECTOR =
  "input[type='checkbox']:is([data-select-row], [data-select-all], [data-js-check], [data-js-check-all])";
const SORT_BUTTON_SELECTOR = ':is([data-sort], [data-js-sort], .dads-table__sort-button)';

export function createTableStyles(tagName: string): CSSStyleSheet {
  return css`
    ${tagName},
    ${tagName} .dads-table {
      margin: 0;
