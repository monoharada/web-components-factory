...
export class DadsDatePicker extends TypographyFormComponent {
  ...
  #syncDisabled(): void {
    ...
    this.#setInputsState(isDisabled, 'disabled', 'data-disabled');
  ...
  #syncReadonly(): void {
    ...
    this.#setInputsState(isReadonly, 'readOnly', 'data-readonly');
  ...
  #syncAriaDescribedBy(): void {
    const externalIds = this.#getExternalDescribedByIds();
  ...
  #refreshExternalAriaDescribedByProxies(): void {
    this.#syncExternalAriaDescribedByProxies(this.#getExternalDescribedByIds());
  }

  #getExternalDescribedByIds(): string[] {
  ...
  async #openCalendar(): Promise<void> {
    ...
    try {
      const ready = await this.#prepareCalendar();
      if (!ready) {
        shouldClose = true;
        return;
      }
      ...
      this.#syncCalendarButtonDisabled();
      if (shouldClose) this.#closeCalendar();
    }
  }
  ...
  #ensureCalendarElement(prefix: string, forceReplace = false): void {
    const popover = this.#calendarPopover;
    const root = this.shadowRoot;
    if (!popover || !root) return;

    const expectedName = `${prefix}-calendar`;
    let ensured = ensurePrefixedElement<HTMLElement>(root, 'calendar', expectedName, forceReplace);

    if (!ensured) {
      ensured = document.createElement(expectedName) as HTMLElement;
      ensured.setAttribute('part', 'calendar');
      ensured.id = 'calendar';
      popover.appendChild(ensured);
    }

    this.#calendar = ensured;
  }

  #setInputsState(
    value: boolean,
    prop: 'disabled' | 'readOnly',
    dataAttr: 'data-disabled' | 'data-readonly',
  ): void {
    const inputs = [this.#yearInput, this.#monthInput, this.#dayInput];
    for (const input of inputs) {
    ...
    if (this.#inputs) {
      this.#inputs.toggleAttribute(dataAttr, value);
    }
  }

  async #prepareCalendar(): Promise<boolean> {
    if (!this.#calendarPrefix) return false;
      ...
      try {
        ...
        if (this.#calendar) {
          ...
          if (typeof customElements.upgrade === 'function') {
            customElements.upgrade(this.#calendar);
          }
        }
        return true;
      } catch (error) {
  ...
  async #getCalendarApi(): Promise<(HTMLElement & DadsCalendarPublicAPI) | null> {
    const calendar = this.#calendar;
    if (!calendar) return null;
    const maybeApi = calendar as unknown as DadsCalendarPublicAPI;
    if (typeof maybeApi.setSelectedDate === 'function') {
      return calendar as HTMLElement & DadsCalendarPublicAPI;
    }
    ...
    if (typeof (calendar as unknown as DadsCalendarPublicAPI).setSelectedDate !== 'function') return null;
    return calendar as HTMLElement & DadsCalendarPublicAPI;
  }
}
