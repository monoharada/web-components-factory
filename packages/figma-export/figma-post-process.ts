 ...
 */
export function convertImagesToDataUri(root: Element): void {
 ...
 */
function relaxFixedHeights(el: HTMLElement): void {
 ...
 */
function insertUnderlineElements(el: HTMLElement): void {
  processUnderlineRecursive(el);
}

function processUnderlineRecursive(el: HTMLElement): void {
 ...
 */
function findTextContainer(el: HTMLElement): HTMLElement | null {
 ...
 *   </div>
 */
function insertFocusRingElements(el: HTMLElement): HTMLElement {
  // outline を持つ要素を探す
  ...
  if (!outlineWidth || outlineWidth === '0px') return el;
 ...
 */
function findOutlineElement(el: HTMLElement): HTMLElement | null {
  const style = el.style.getPropertyValue('outline-style');
  if (style && style !== 'none') return el;
 ...
 */
function parseSpreadOnlyShadow(value: string): SpreadShadow | null {
 ...
 */
function addPx(radius: string, add: number): string {
